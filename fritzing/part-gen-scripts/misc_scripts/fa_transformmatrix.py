...
inputDir = "C:\\Users\\joev\\Documents\\schematic-svg-transform\\input"
outputDir = "C:\\Users\\joev\\Documents\\schematic-svg-transform\\output\\"
...
def myFilter(svgFilename, filename):
	...
	transforms = set() #make a set (set is like a list but without doubled same entries)
	for child in parent_map.values():
		removed = False
		# print "child", child
		id = child.attrib.get('transform')
		if id and id.startswith('="matrix(1 0 0 1 '): #get the child ids to decide if we need to clean or not
		...
		for transform in transforms:
			paths.extend(transform.findall("text")) #writes the list where are path-elemnts are in that are in a parent called "copper"
			
			#print "paths", paths
			for text in textss:
				#print "path", path
				#print "parent_map", parent_map
				id = text.attrib.get("transform")
				
				if id is None:
					try:
						#print "id", id
						parent_map[text].remove(text) #remove the child from his parent
						removed = True
