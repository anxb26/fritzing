...
def myFilter(svgFilename, filename):
	...
	matrixTransforms = set() #make a set (set is like a list but without doubled same entries)
	for child in parent_map.values():
		removed = False
		# print "child", child
		id = child.attrib.get("id")
		if id and id.startswith('schematic'): #get the child ids to decide if we need to clean or not
			matrixTransforms.add(child)
			
			
		paths = list() 
		
		for matrix in matrixTransforms:
			paths.extend(matrix.findall('<text transform="matrix(1 0 0 1")') #writes the list where are path-elemnts are in that are in a parent called "copper"
